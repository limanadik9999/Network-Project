...
class Status(models.Model):
    ...
    def get_online_status(self):
        ...
        if user_online.date() == (datetime.now() - timedelta(days=1)).date():
             ...
             status = 'Was online ' + user_online.date().strftime("%d.%m") + ' at ' + user_online.time().strftime("%H:%M")
        else:
             status = 'Was online ' + user_online.date().strftime("%d.%m.%Y") + ' at ' + user_online.time().strftime("%H:%M")
